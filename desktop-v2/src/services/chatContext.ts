...
const TOOL_GUIDANCE = [
  ...
  "- get_recent_screen_activity(limit?, since_minutes?, app_name_contains?) — pulls the literal OCR text of recent on-screen content. ALWAYS call this for any question about what the user just typed/asked/said/saw/did, or what is on a specific app right now, OR ANY 'screen time' / 'what have I been doing' / 'summarize my activity' style request. Use since_minutes to scope ('1 minute ago' → since_minutes=2 to be safe; 'screen time' / 'today' → since_minutes=1440 OR omit it). Use app_name_contains to target an app ('Claude Code' lives inside Ghostty/terminal apps; browsers are 'chrome'/'safari'/'arc'). If the response says 'No screen activity matched the requested filter' but reports 'history: N+ screenshots in DB', RETRY with a wider since_minutes (or omit it) before telling the user there's no data. Only conclude Rewind isn't tracking when the response literally says 'Rewind has never recorded'. Never say you can't see recent activity without calling this first.",
  "",
