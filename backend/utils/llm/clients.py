...
llm_persona_mini_stream = ChatOpenAI(
    ...
    default_headers={"X-Title": "Omi Chat"},  # allow-omi: OpenRouter analytics header, not user-visible
    streaming=True,
    stream_options={"include_usage": True},
    callbacks=[_usage_callback],
)
...
llm_gemini_flash = ChatOpenAI(
    ...
    api_key=os.environ.get('OPENROUTER_API_KEY'),
    base_url="https://openrouter.ai/api/v1",
    default_headers={"X-Title": "Omi Wrapped"},  # allow-omi: OpenRouter analytics header, not user-visible
    callbacks=[_usage_callback],
)
